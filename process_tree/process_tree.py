...
Can be used as a standalone script or as a UDB command.

This code was generated by Claude Code and has had only minimal human review and testing.
"""
...
class ProcessTree:
    ...
    def build_relationships(self) -> bool:
...
class SVGRenderer:
    ...
    def _calculate_layout(self, tree: ProcessTree) -> Dict[int, LayoutInfo]:
        ...
        def assign_y_positions(process: Process, y: int):
            nonlocal current_y
            y_positions[process.pid] = y
            current_y = max(current_y, y + 1)

            for child in process.children:
                assign_y_positions(child, current_y)
                current_y += 1

        assign_y_positions(tree.root, 0)
    ...
    def _draw_process_lines(
        self, svg: ET.Element, processes, layout: Dict[int, LayoutInfo]
    ) -> None:
...
class ProcessTreeVisualizer:
    ...
    def load_and_visualize(
        ...
        if not tree.root:
            print("Could not build process tree. Exiting.", file=sys.stderr)
            return
    ...
    def _load_recordings(self, recordings_dir: Path) -> ProcessTree:
        ...
        if not recording_files:
            print(f"No .undo files found in {recordings_dir}", file=sys.stderr)
            return ProcessTree()
        ...
        if not tree.build_relationships():
            print("Failed to build process relationships", file=sys.stderr)
